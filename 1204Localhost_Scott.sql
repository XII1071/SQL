...
SELECT  DEPTNO, JOB, COUNT(*),
        MAX(SAL), SUM(SAL), TRUNC(AVG(SAL), 2)
FROM EMP
...
-- GROUPING 예제
SELECT DECODE(GROUPING(DEPTNO), 1, 'ALL-DEPT', DEPTNO)AS DEPTNO,
        DECODE(GROUPING(JOB), 1, 'ALL-JOB', JOB)AS JOB,
        COUNT(*), MAX(SAL), SUM(SAL)
FROM EMP
GROUP BY CUBE(DEPTNO, JOB)
ORDER BY DEPTNO, JOB;
...
SELECT DECODE(GROUPING_ID(JOB), 2, 'ALL-DEPT', 3, 'ALL-DEPT', DEPTNO)AS DEPTNO,
        DECODE(GROUPING_ID(JOB), 1, 'ALL-JOB', 3, 'ALL-JOB', JOB)AS JOB, COUNT(*)
FROM EMP
GROUP BY CUBE(DEPTNO, JOB)
ORDER BY DEPTNO, JOB;
